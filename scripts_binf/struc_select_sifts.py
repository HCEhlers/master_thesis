...
def build_strucmap(target_record, min_res=40, n_shortlist=1000, min_ping_time=5.0, verbose=0):
    """Build a structure map from SIFTS URL requests"""
    
    target_id = target_record.id
    
    strucmap = url_sifts_strucmap(target_record, min_ping_time=min_ping_time, verbose=verbose)

    n_struc_raw = len(strucmap.index)
    if n_struc_raw < 1:
        raise ValueError("SIFTS returned empty map")
    ...
    if n_struc_minres > 0 and n_struc_minres > n_shortlist:
        if verbose > 0:
            print("%s: Shortlisting from %d to top %d based on SIFTS assignments alone" %
                  (target_id, n_struc_minres, n_shortlist))
        strucmap = rank_structures(target_record, strucmap, verbose=verbose)
        strucmap = strucmap.iloc[range(n_shortlist)]
        strucmap.reset_index(inplace=True, drop=True)
    n_struc_sl = len(strucmap.index)
            
    # Report size of structure map
    if verbose > 0:
        print("%s structure map build summary: sifts_raw: %d  non-redundant: %d  min_res_%d: %d  shortlist: %d" %
              (target_id, n_struc_raw, n_struc_nr, min_res, n_struc_minres, n_struc_sl))
...
def url_sifts_strucmap(target_record, min_ping_time=5.0, verbose=0):
    ...
    columns = ['pdb', 'chain', 'score', 'method_exp', 'method_res', 'unp_start', 'unp_end', 'resseq_start', 'resseq_end', 'cover_exp']
    
    sifts_dict = {'pdb': [d['pdb_id'] for d in sifts_list],  'chain': [d['chain_id'] for d in sifts_list], 'score': [0.0]*n_struc, 
...
def align_strucmap(target_record, strucmap, min_ping_time=5.0, verbose=0):
    ...
    # counting/debugging variable - change to farst forwad in alignments
    c = 0
    for smap in list(strucmap.itertuples(index=False))[c:]:
        ...
        try:
            ...
            # np.nan is type float and gives problems - cover_obs=0.0 and/or resseq="" could be missing signal
            cover_obs_col.append(0.0)
            ident_exp_col.append(0)
            ident_obs_col.append(0)
            inserts_col.append(0)
            deletions_col.append(0)
            mismatch_col.append(0)
            non_obs_col.append(0)
            modified_col.append(0)
            resseq_col.append("-")
            aligned_resseq_col.append("-")
            aligned_resseq_obs_col.append("-")
            continue
    ...
    if len(cover_obs_col) != len(strucmap.index):
        raise ValueError("ERROR: Only %d of %d rows in structure map have cover_obs determined (didn't check all other columns)" % (len(cover_obs_col), len(strucmap.index)))
...
if __name__ == '__main__':
    ...
    # Minimum number of residues reuired for a structure to be relevant
    min_res = 30

    # Shortlist structure map based on SIFTS coverage before looking up PDB RESSEQ and obsereved 
    n_shortlist = 1000

    overwrite_data_files = False

    # File to write with selected structures
    outfile = "./cytosol_structures.csv"
    
    # File containing list of uniprot ids
    infile = '/storage1/holger/master_thesis/scripts/cytosol_test.csv'

    data_dir = "/storage1/holger/master_thesis/data/"
    ...
    # Output file with structures for all input targets
    if outfile[(-4):] != ".csv":
        outfile = outfile + ".csv"
    if os.path.isfile(outfile):
        c = 0
        backup_outfile = outfile[:outfile.rfind(".")] + "_backup%03d.csv" % (c)
        while os.path.isfile(backup_outfile):
            c += 1
            backup_outfile = outfile[:outfile.rfind(".")] + "_backup%03d.csv" % (c)
        print("Output file %s excists. Moveing old file to %s" % (outfile, backup_outfile))
        os.rename(outfile, backup_outfile)
        
    outfile_handle = open(outfile,'tw')
    outfile_handle.write("%12s;  %6s;  %4s;  %5s\n" % ("target_id", "struc", "N", "cover"))
    ...
    # Read input csv file of uniprot entries
    if not os.path.isfile(infile):
            print("ERROR: Could not open file with uniprot entries: %s" % (infile))
            sys.exit(2)        
    gene_list_df = pd.read_csv(infile,  comment='#')
    uniprot_id_list = list(gene_list_df['Entry'])

    # # DEVELOPMENT: Overwrite input file with new uniprot_id_list
    ...
    # # Single uniprot for debugging
    # uniprot_id_list = ['Q9BY41']
    if verbose > 0:
    ...
    for uniprot_id in uniprot_id_list:
        ...
        if not has_strucmap_file or overwrite_data_files:
            ...
            try:
                strucmap = build_strucmap(uniprot_record, min_res=min_res, n_shortlist=n_shortlist, min_ping_time=min_ping_time, verbose=verbose)
            except Exception as err:
                print("WARNING: %s structure map building failed (%s)" % (uniprot_id,err))
            ...
            try:
                ...
                print("WARNING: %s PDB alignments failed (%s)" % (uniprot_id,err))

        else:
            if verbose > 0:
                print("%s: Getting structure map from file %s" % (uniprot_id, strucmap_filename))
            # Read struncture map from disk - no knowledge of applied filters (min_res and shortlist)
            strucmap = pd.read_csv(data_dir + "/" + strucmap_filename, sep=';')
            
        # Check that strutures were found
        n_struc = len(strucmap.index)
        if n_struc < 1:
            print("%s: No structures found" % (uniprot_id))
            # Write result to output file
            # Consider adding a name or (first part of) description
            # Consider adding number of target residues, covered residues by selected structure and covered residues by all structures
            outfile_handle.write("%12s;  %6s;  %4d;  %5.3f\n" % (uniprot_id, "-", 0, 0.0))
            outfile_handle.flush()
            continue
            
        print("%s: Got structure map with %d chains" % (uniprot_id, n_struc))

        # Columns for pretty-printing
        pcol = ["pdb", "chain", "score", "method_exp", "method_res", "cover_exp", "cover_obs", "ident_exp", "ident_obs",
        ...
        # Write result to output file
        if 'cover_obs' in strucmap.columns:
            outfile_handle.write("%12s;  %4s-%1s;  %4d;  %5.3f\n" %
                                 (uniprot_id, strucmap.loc[0,'pdb'], strucmap.loc[0,'chain'], n_struc, strucmap.loc[0,'cover_obs']))
        else:
            outfile_handle.write("%12s;  %4s-%1s;  %4d;  %5.3f\n" %
                                 (uniprot_id, strucmap.loc[0,'pdb'], strucmap.loc[0,'chain'], n_struc, 0.0))
        # push to OS buffer. If not readable after this, force OS to empty buffer with os.fsync(outfile_handle)
        outfile_handle.flush()
        
        # Write aligned sequences to FASTA file
        if 'aligned_resseq_obs' in strucmap.columns:
        ...
        # Break here if map os not aligned
        if not 'cover_obs' in strucmap.columns:
            continue
            
        print("%s: best structure %s-%s of %d method: %s  cover_obs: %.3f  deletions: %d  inserts: %d  mismatch: %d  modified: %d" %
              (uniprot_id, strucmap.loc[0,'pdb'], strucmap.loc[0,'chain'], n_struc, strucmap.loc[0,'method_exp'], strucmap.loc[0,'cover_obs'],
        ...
        # print( [s for s in strucmap['aligned_resseq_obs'] ] )
